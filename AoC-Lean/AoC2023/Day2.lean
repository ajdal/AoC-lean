...
instance : Add Draw where
  add := fun d1 d2 => {
    red := d1.red + d2.red,
    green := d1.green + d2.green,
    blue := d1.blue + d2.blue
  }
...
def isValid : GameS → Bag → Bool :=
  fun g b =>
    g.draws.all (fun d =>
        d.red ≤ b.red ∧ d.green ≤ b.green ∧ d.blue ≤ b.blue
    )

def d1 : Draw := {red := 3, green := 2, blue := 1}
def d2 : Draw := {red := 1, green := 2, blue := 3}

def join : Draw → Draw → Draw :=
  fun d1 d2 =>
    {
...
notation d1 "∨" d2 => join d1 d2

def Draw.power : Draw → Nat := fun d => d.red * d.green * d.blue

declare_syntax_cat color
syntax num "red" : color
syntax num "green" : color
syntax num "blue" : color

macro_rules
  | `(color| $x:num red) => `(Draw.mk $x 0 0)
  | `(color| $x:num green) => `(Draw.mk 0 $x 0)
  | `(color| $x:num blue) => `(Draw.mk 0 0 $x)

instance : Coe (TSyntax `color) (TSyntax `term) where
  coe s := ⟨s.raw⟩

syntax "Game " num ": " sepBy1(sepBy1(color, ","), ";")"." : term
macro_rules
  | `(Game $id:num : $[$[$colorss],*];*.) => do
      let colorss ← colorss.mapM (
        fun colors => do
          let colors : Array (TSyntax `term) := colors.map (fun c => c)
          let initial ← `(Draw.mk 0 0 0)
          let draw ← colors.foldlM (fun acc t =>
            `($acc + $t)
          ) initial
          `($draw)
        )
...
def runDay (games : List GameS) : String :=
    -- Part 1
    let bag : Bag := Bag.mk 12 13 14
    let valid := games.filter (fun game => isValid game bag)
    let ids := valid.map GameS.id
    let sum := Util.sum 0 ids
    let p1 := s!"Sum of valid game ids: {sum}\n"

    -- Part 2
    let gamePows := games.map (fun game =>
        (game.draws.foldl join {}).power
    )
    let sum := Util.sum 0 gamePows
    let p2 := s!"Sum of powers: {sum}"
    p1 ++ p2
